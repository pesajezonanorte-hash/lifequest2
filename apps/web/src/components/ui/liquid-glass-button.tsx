...
// liquid-glass-button.tsx — componentes estilo shadcn (LiquidButton / MetalButton / Button).
//
// Integración en LifeQuest (apps/web):
// - Ruta por defecto de componentes: /components/ui (shadcn) → apps/web/src/components/ui/ ✓
// - Helper cn en @/lib/utils (clsx + tailwind-merge) ✓
// - Tokens semánticos shadcn (primary, destructive, ring, input…) añadidos a
//   tailwind.config.ts y mapeados a las CSS vars del tema en globals.css ✓
// - Dependencias: @radix-ui/react-slot, class-variance-authority ✓
// - Adaptaciones mínimas respecto al original:
//   * `bg-linear-to-t` (Tailwind v4) → `bg-gradient-to-t` (v3.4, el stack actual)
//   * `dark:inset-shadow-*` es utilidad de v4: se conserva, en v3 se ignora sin romper
//   * El bloque `export {...}` se movió al final del archivo (declaraciones primero)
...
const liquidbuttonVariants = cva(
  "inline-flex items-center isolate justify-center cursor-pointer gap-2 whitespace-nowrap rounded-full text-sm font-medium transition-all duration-300 ease-out disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-accent-purple/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        default: "bg-transparent hover:scale-110 text-primary",
        destructive:
...
// Exportados al final para declarar antes de exportar (el original los exportaba
// a mitad de archivo, antes de las declaraciones).
